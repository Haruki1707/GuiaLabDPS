...
class PantallaInicio extends React.Component {
    ...
    Entrar() {
        if (!!this.state.usuario && !!this.state.contrasena) {
            fetch('https://dpsmarket.000webhostapp.com/apiusuario.php?comando=autenticar&usuario=' + this.state.usuario + '&contrasena=' + this.state.contrasena, {
                method: 'GET'
    ...
    render() {
        return (<View style={{flex: 1, padding: 10}}>
            ...
            >
                <Text style={{
                    color: 'white', fontSize: 22, textAlign: 'center', textAlignVertical: 'center'
                }}>Entrar</Text>
            </TouchableOpacity>
        </View>);
    }
...
class listarProductos extends React.Component {
    ...
    cargarRegistros() {
        console.log('Prueba');
        fetch('https://dpsmarket.000webhostapp.com/api.php?comando=listar', {
            method: 'GET'
        })
            .then((response) => response.json())
            .then((responseJson) => {
                ...
                console.log(listado);
                this.setState({
                    elementos: listado, total: listado.length
                })
            })
            .catch((error) => {
                console.error(error);
    ...
    render() {
        return (<View style={{flex: 1}}>
            <NavigationEvents onWillFocus={() => {this.cargarRegistros();}}/>
            <Text
                style={{
                    fontSize: 18,
                    textAlign: 'center',
...
class PaginaDetalle extends React.Component {
    static navigationOptions = {
        title: 'Editar producto', headerStyle: {
            backgroundColor: '#f4511e',
        }, headerTintColor: '#fff', headerTitleStyle: {
            fontWeight: 'bold',
        },
    ...
    state = {
        nombre: '', descripcion: '', cantidad: '', preciodecosto: '', preciodeventa: '', fotografia: '', id: ''
    }

    Actualizar() {
        fetch('https://dpsmarket.000webhostapp.com/api.php?comando=editar&nombre=' + this.state.nombre + '&descripcion=' + this.state.descripcion + '&cantidad=' + this.state.cantidad + '&preciodecosto=' + this.state.preciodecosto + '&preciodeventa=' + this.state.preciodeventa + '&fotografia=' + this.state.fotografia + '&id=' + this.state.id, {
            method: 'GET'
        }).then((response) => response.json())
            .then((responseJson) => {
                console.log(responseJson);
                ...
                console.log(mensaje);
                if (!mensaje) alert("Error al actualizar!"); else {
                    alert(mensaje);
    ...
    Eliminar() {
        fetch('https://dpsmarket.000webhostapp.com/api.php?comando=eliminar&id=' + this.state.id, {
            method: 'GET'
        }).then((response) => response.json())
            .then((responseJson) => {
                console.log(responseJson);
                const mensaje = responseJson.mensaje;
                console.log(mensaje);
                if (!mensaje) alert("Error al eliminar!"); else {
                    alert(mensaje);
                    this.props.navigation.goBack();
                }
            })
    ...
    render() {
        ...
        return (<View style={{flex: 1}}>
                <ScrollView style={{flex: 1}}>
                    <View
                        style={{
                            flex: 1, flexDirection: 'row', alignContent: 'center', alignItems: 'center', height: 60
                        }}>
                        <TouchableOpacity
                            style={{
                                flex: 1,
...
class PaginaAgregar extends React.Component {
    static navigationOptions = {
        title: 'Agregar producto', headerStyle: {
            backgroundColor: '#f4511e',
        }, headerTintColor: '#fff', headerTitleStyle: {
            fontWeight: 'bold',
        },
    ...
    state = {
        nombre: '', descripcion: '', preciodeventa: '', preciodecosto: '', cantidad: '', fotografia: '',
    }

    Guardar() {

        fetch('https://dpsmarket.000webhostapp.com/api.php?comando=agregar&nombre=' + this.state.nombre + '&descripcion=' + this.state.descripcion + '&cantidad=' + this.state.cantidad + '&preciodecosto=' + this.state.preciodecosto + '&preciodeventa=' + this.state.preciodeventa + '&fotografia=' + this.state.fotografia, {
            method: 'GET'
        })
            .then((response) => response.json())
            .then((responseJson) => {
                ...
                const mensaje = responseJson.mensaje;
                console.log(mensaje);
                if (!mensaje) alert("Error al agregar!"); else {
                    alert(mensaje);
                    this.props.navigation.goBack();
                }
            })
...
const RootStack = createStackNavigator({
    Inicio: PantallaInicio, ListarProductos: listarProductos, Detalles: PaginaDetalle, Agregar: PaginaAgregar,
}, {
